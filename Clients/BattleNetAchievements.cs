    ...
    class BattleNetAchievements : GenericAchievements
    {
        ...
        public override GameAchievements GetAchievements(Game game)
        {
            try
            {
                ...
                if (game.Name.ToLower() == "starcraft ii" || game.Name.ToLower() == "starcraft 2")
                {
                    return GetAchievementsStarCraft2(game);
                }
            }
            catch (Exception ex)
            {
        ...
        #region StarCraft II
        private GameAchievements GetAchievementsStarCraft2(Game game)
        {
            List<Achievements> AllAchievements = new List<Achievements>();
            List<GameStats> AllStats = new List<GameStats>();
            string GameName = game.Name;
